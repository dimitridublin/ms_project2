...
* Below the company's price promise users are presented with a stylish booking menu in order to preselect their criteria for the packages the website should display to them. This functionality was not further developed (i.e. "book now" button is defunct) as a) it was outside the scope of this project, and b) it required a database of actual accommodation packages which BTT as an imaginary company does not have.
