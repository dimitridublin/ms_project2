...
* Testing of booking form on Book page:
  * Tried to submit the form without all fields completed and verified that an error message regarding each missing field appears
  * Tried to submit the form with all inputs valid and verified that on submission form generates a message that functionality is under construction and prompts user to contact company by phone or email instead (as form submission function was not due to be developed during this project, see screenshot below).
...
* When setting up the map rendering, the callback function in the default Google script source (i.e. callback of initMap) did not work and map was not being rendered. As a result, and following a tutor's advice, callback was removed from the script source and calling of function initMap was coded into the separate js file connected to the Google API instead. This solved the map rendering issue, however, the JavaScript related code then started throwing some inexplicable errors. When contacted about them, tutor support advised to start the process again from the very beginning rather than trying to fix those errors. As this caused the reappearance of the original problem though (i.e. map not rendering in the first place), I decided to proceed with the callback decoupled from the script source (i.e. script source ends in the API key in the respespective HTML files and initMap function is called from the JS files). This somehow limited my ability to add more interactivity to the city maps (such as displaying of additional location info when a marker was clicked, etc.) as trying to do so was bringing back those other errors.
